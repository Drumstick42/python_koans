...
class AboutStringManipulation(Koan):
    ...
    def test_you_can_get_a_single_character_from_a_string(self):
        string = "Bacon, lettuce and tomato"
        self.assertEqual('a', string[1]) # Returns a string, not a char type. No char type in python
    ...
    def test_strings_can_be_joined(self):
        ...
        #String that join is called on is used as the padding between elements

    def test_strings_can_change_case(self):
        self.assertEqual('Guido', 'guido'.capitalize()) # Only capitalizes the first
        self.assertEqual('GUIDO', 'guido'.upper())
        self.assertEqual('timbot', 'TimBot'.lower())
        self.assertEqual('Guido Van Rossum', 'guido van rossum'.title()) # Capitalizes all words.
