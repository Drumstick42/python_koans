...
class AboutLists(Koan):
    ...
    def test_accessing_list_elements(self):
        ...
        self.assertEqual('jelly', noms[-1]) # Access last list element
        self.assertEqual('butter', noms[-3]) # Second element (2 from last)
    ...
    def test_slicing_lists(self):
        ...
        self.assertEqual([], noms[2:2]) #Slice end is non-inclusive, so empty
        self.assertEqual(['and', 'jelly'], noms[2:20])
        self.assertEqual([], noms[4:0]) # Doesn't look like we can go backward
        self.assertEqual([], noms[4:100]) # So, we can access out of range indices w/o error
        self.assertEqual([], noms[5:0])

    def test_slicing_to_the_edge(self):
        noms = ['peanut', 'butter', 'and', 'jelly']

        self.assertEqual(['and', 'jelly'], noms[2:]) # Leave off index to go to last or first element
        self.assertEqual(['peanut', 'butter'], noms[:2])

    def test_lists_and_ranges(self):
        #range produces an immutable set of numbers. Not a list, but can be 
        # converted. range(start, stop[, step])
    ...
    def test_insertions(self):
        #Insert places argument element at the specified index.
    ...
    def test_popping_lists(self):
        stack = [10, 20, 30, 40]
        stack.append('last') # Puts at end. Note we can mix types on a list

        self.assertEqual([10, 20, 30, 40, 'last'], stack)

        popped_value = stack.pop() # Removes from end and returns, just like a stack
        self.assertEqual('last', popped_value)
        self.assertEqual([10, 20, 30, 40], stack)

        popped_value = stack.pop(1) # Removes from index 1
